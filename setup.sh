...
if [ "$(grep -Ei 'debian|buntu|mint' /etc/*release)" ]; then
    ...
    git clone https://github.com/paciente23256/mail-headers-analyze.git /var/mail-analyzer
    ...
    if [ "$os_version" == 8 ]; then
        ...
        sudo pip3 install virtualenv
        git  -c http.sslVerify=false clone https://github.com/paciente23256/mail-headers-analyze.git /var/mail-analyzer
        cd /var/mail-analyzer
        sudo pip3 install -r requirements.txt
        sudo virtualenv virt
        sudo source virt/bin/activate
